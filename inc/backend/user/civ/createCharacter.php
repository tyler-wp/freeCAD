...
//Check if they have the max amount of characters Created
...
if ($myCharCount === $settings['civ_char_limit']) {
  $error['msg'] = "You have reached your limit of civilian characters, and can not create anymore.";
...
if (strlen($newChar['first_name']) < 2) {
    $error['msg'] = "Your first name must be longer then 2 characters.";
    echo json_encode($error);
    exit();
} elseif (strlen($newChar['last_name']) > 120) {
    $error['msg'] = "Your last name must be longer then 2 characters.";
    echo json_encode($error);
    exit();
}

if ($newChar['gender'] === "Select Gender") {
  $error['msg'] = "Opps! Please select a Gender.";
  echo json_encode($error);
  exit();
} elseif ($newChar['race'] === "Select Race") {
  $error['msg'] = "Opps! Please select a Race.";
  echo json_encode($error);
  exit();
}
