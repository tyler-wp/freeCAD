function getUserCharacters() {
    fetch('inc/backend/user/civ/getCharacters.php').then(function(response) {
        response.text().then(function(text) {
            $('#listCharacters').html(text);
        });
    });
}
...
function stolenVehicle(str) {
    var i = str.id;

    $.ajax({
        url: "inc/backend/user/civ/stolenVehicle.php?id=" + i,
        cache: false,
        success: function(error) {
            var error = JSON.parse(error);
            if (error['msg'] === "") {
                toastr.success('Stolen Status Updated', 'System:', {
                    timeOut: 10000
                });
            } else {
                toastr.error(error['msg'], 'System:', {
